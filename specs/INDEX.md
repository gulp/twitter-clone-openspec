...
- [security-sql-injection-prevention.md](security-sql-injection-prevention.md) — Prisma $queryRaw safety, Prisma.sql tagged templates, parameter escaping guarantees, raw SQL injection prevention
- [security-password-reset-tokens.md](security-password-reset-tokens.md) — Password reset flow with SHA-256 token hashing, SELECT FOR UPDATE race prevention, timing-attack resistance, single-use enforcement
...
- [engagement-batch-state-check.md](engagement-batch-state-check.md) — Batch hasLiked/hasRetweeted checks via two parallel IN queries, Set conversion for O(1) lookup, N+1 query prevention, Map vs Set patterns
